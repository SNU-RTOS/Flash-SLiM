...
def _capture_phase_raw_record(
    pid: int, phase_name: str, op_index: int, obj_index: int, start_ns: int, end_ns: int
) -> PhaseRaw:
    ...
    # print(f"Captured phase: pid={pid}, phase={phase_name}, op_index={op_index}, obj_index={obj_index}, start_ns={start_ns}, end_ns={end_ns}")
    return PhaseRaw(
        pid=pid,
...
def on_phase_event(cpu, data, size):
    ...
    raw = _capture_phase_raw_record(e.pid, name, op_index, obj_index, start, e.ts)
...
def print_report():
    ...
    if cnt:
        ...
        print(
            f"{'Phase':50s}  {'Count':>5s}  {'Avg(ms)':>10s}  {'Min(ms)':>10s}  {'Max(ms)':>10s}  {'Total(ms)':>12s}"
        )
        for k in items:
...
if __name__ == "__main__":
    ...
    b["events"].open_ring_buffer(on_phase_event)
